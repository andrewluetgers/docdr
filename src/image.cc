...
NAN_MODULE_INIT(Image::Init)
{
    ...
    Nan::SetPrototypeMethod(ctor, "niblackAdaptiveThreshold", NiblackAdaptiveThreshold);
    Nan::SetPrototypeMethod(ctor, "sauvolaAdaptiveThreshold", SauvolaAdaptiveThreshold);
...
//    Nan::SetPrototypeMethod(ctor, "baselines", Baselines);
...
NAN_METHOD(Image::NiblackAdaptiveThreshold)
{
    Image *obj = Nan::ObjectWrap::Unwrap<Image>(info.Holder());
    ...
    if (info[0]->IsInt32() && info[1]->IsInt32() && info[2]->IsNumber()) {
        ...
        int32_t winy = info[1]->Int32Value();
        float optK = static_cast<float>(info[2]->NumberValue());
        cv::Mat converted = binarizeNiblack(pix8ToMat(obj->pix_), winx, winy, optK);
        Pix *pixd = mat8ToPix(&converted);
...
NAN_METHOD(Image::SauvolaAdaptiveThreshold)
{
    Image *obj = Nan::ObjectWrap::Unwrap<Image>(info.Holder());
...
//	int error = pixGetRegionsBinary(pix, &pixh, &pixl, &pixp, 0);
//
//	Numa na = pixFindBaselines(pix, &pta, 1);
//
//
//
//	if (error == 0) {
