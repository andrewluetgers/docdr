...
void NiblackSauvolaWolfNick (Mat im, Mat output, NiblackVersion version,
	int winx, int winy, double k, double r) {
 ...
 **********************************************************
 ...
 winX: int
	 Odd size width of pixel neighborhood window (e.g. 3, 5, 7...)
 winY: int
	 Odd size height of pixel neighborhood window (e.g. 3, 5, 7...)
 optK: float, optional
	 Value of the positive parameter k.
 optR: float, optional
	 Value of R, the dynamic range of standard deviation.
	 If None, NiblackSauvolaWolfNick function will set it to
	 half of the image intensity min, max delta.
 */
Mat binarize (Mat input, NiblackVersion versionCode, int winx, int winy, float optK, double optR) 
{
	...
	// limit window size to max of 99
	if (winx > 99) {
		winx = 99;
	}
	if (winy > 99) {
	...
	NiblackSauvolaWolfNick(input, output, versionCode, winx, winy, optK, optR);
...
{
	return binarize(input, NIBLACK, winx, winy, optK, NULL);
}

Mat binarizeSauvola (Mat input, int winx, int winy, float optK, double optR)
{
	return binarize(input, SAUVOLA, winx, winy, optK, optR);
}

Mat binarizeWolf (Mat input, int winx, int winy, float optK)
{
	return binarize(input, WOLFJOLION, winx, winy, optK, NULL);
}

Mat binarizeNick (Mat input, int winx, int winy, float optK)
{
	return binarize(input, NICK, winx, winy, optK, NULL);
}